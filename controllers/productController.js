...
exports.getProductBySKU = async (req, res) => {
  await doActionThatMightFailValidation (req, res, async () => {
      ...
      const products = await ProductService.getProductBySKU({SKU: SKU});
...
exports.deleteProductBySKU = async(req, res) => {
  await doActionThatMightFailValidation(req, res, async () => {
    const {SKU} = req.params;
    const result = await ProductService.deleteProductBySKU({SKU: SKU});
    if (result?.deleteCount > 0) {
...
exports.patchProductBySKU = async(req, res) => {
  await doActionThatMightFailValidation(req, res, async () => {
    ...
    const product = req.body;
    delete product.SKU;
    const result = await ProductService.patchProductBySKU({SKU: SKU}, product);
    if (result?.deleteCount > 0) {

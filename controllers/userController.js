...
async function doActionThatMightFailValidation(req, res, action) {
  try {
    await action();
  } catch (e) {
...
exports.getUserBySSN = async (req, res) => {
  await doActionThatMightFailValidation(req, res, async () => {
    ...
    const users = await UserService.getUserBySSN({ SSN });
...
exports.deleteUserBySSN = async (req, res) => {
  await doActionThatMightFailValidation(req, res, async () => {
    const { SSN } = req.params;
    const result = await UserService.deleteUserBySSN({ SSN });
    if (result?.deleteCount > 0) {
...
exports.patchUserBySSN = async (req, res) => {
  await doActionThatMightFailValidation(req, res, async () => {
    ...
    const user = req.body;
    delete user.SSN;
    const result = await UserService.patchUserBySSN({ SSN }, user);
    if (result?.deleteCount > 0) {
